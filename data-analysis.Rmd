...
In the following analysis, we need a way to quantify how crowded is a certain road. To acheive this we will be using the user's reports. People's reports are one of the followings:
```
1 --> 7lawa
2 --> lazez
3 --> mashy
...
```{r}
s7rawy_da2ery <- data %>% filter(rd.nm == "Me7war;Sa7rawy To Da2ery")
sa7rawy_da2ery_speed <- speed_data %>% filter(from == "Me7war;Sa7rawy" & to == "Me7war;Da2ery")


p1 <- sa7rawy_da2ery %>% filter(!is.na(rd.rp.stid) & rd.rp.stid < 6) %>% separate(report_date, into=c("report_date","report_time"), sep =" ") %>% mutate(report_time=round_time(report_time,2)) %>% group_by(report_time) %>% summarize(c= mean(rd.rp.stid)) %>% ggplot(aes(x=report_time, y=c)) + geom_line()

p2 <- sa7rawy_da2ery_speed %>% separate(report_date, into=c("report_date","report_time"), sep =" ") %>% mutate(report_time=round_time(report_time,2)) %>% group_by(report_time) %>% summarize(c= mean(speed)) %>% ggplot(aes(x=report_time, y=c)) + geom_line()

multiplot(p1,p2)
```

Yes! The two peaks of the rush hour are reflected on this plot, where it experinces a slowdown and an increase of the average stid.
...
```{r}
speed_data %>% separate(report_date, into=c("report_date","report_time"), sep =" ") %>% mutate(report_time=round_time(report_time,60)) %>%  ggplot(aes(x=factor(report_time), y=speed, color=city)) + geom_boxplot() + scale_y_log10()
```

No. The average speed in Cairo is almost higher than Alex.
...
My hypothesis here would be that people tend to report more when it's crowded. To prove this, we'll need to prove that distribution of the average status ID aganist the time is equivilant to the distribtution of the number of reports aganist the time. Which means that at a certain point of time, if the number of reports is hight, then the average status id should be high.
...
p1 <- ggplot(average_stid, aes(x=report_time, y=average_stid/sum(average_stid))) + geom_point()
p2 <- ggplot(reports_per_time, aes(x=report_time, y=c/sum(c))) + geom_point()

multiplot(p1,p2)

ggplot(NULL,aes(y=sort(average_stid$average_stid), x=sort(reports_per_time$c))) + geom_point() + geom_smooth(method="lm", se=FALSE)
```

The qqplot shows that as the number of reports increase, the average stid increase. When the number of reports is greater than 200 report/5 mins, they are both directly proportional and given the number of votes you can directly predict the average stid.