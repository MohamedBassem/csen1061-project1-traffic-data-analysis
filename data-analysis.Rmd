...
round_time <- function(time, mins){
  ...
  as.POSIXct(strptime("1970-01-01", "%Y-%m-%d", tz="UTC") + round(as.numeric(posix_time)/(mins*60))*(mins*60) + 60*60*2) # The added 60*60*2 is a hack to fix the TZ
}

data %>% separate(crawl_date, into=c("crawl_date","crawl_time"), sep =" ") %>% mutate(crawl_time=round_time(crawl_time,30)) %>% group_by(crawl_time) %>% summarize(c=length(rd.rp.cmid)) %>% ggplot(aes(x=crawl_time, y=c)) + geom_bar(stat='identity')
```
...
I though that the gender may be a useful feature in our data so I got the first name of all the users and wrote a script to query an endpoint that can infer genders and select only genders with high confidence. Let's add these info to our data.

```{r}
write.csv(data$rd.rp.nm, file="./name_manipulation/names.txt")
```
...
Now that we have augmented our data with the gender we can start our analysis.
...
have_a_gender %>% separate(report_date, into=c("report_date","report_time"), sep =" ") %>% mutate(report_time=format(round_time(report_time,30), "%H:%M")) %>% group_by(report_time, gender) %>% summarize(c=length(rd.rp.cmid)) %>% ggplot(aes(x=report_time, y=c)) + geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + facet_wrap(~gender, nrow=2) 
```


# Suggested Hypotheses 

### Alex vs Cairo ( in timings probably )

```{r}

report_count <- data.frame(
  cairo = length(data[data$city == "cairo","rd.rp.cmid"]),
  alex = length(data[data$city == "alex","rd.rp.cmid"])
)
```

```{r}
data %>% filter(rd.rp.stid < 6) %>% separate(report_date, into=c("report_date","report_time"), sep =" ") %>% mutate(report_time=format(round_time(report_time,30), "%H:%M")) %>% group_by(report_time, city, rd.rp.stid) %>% summarize(c=length(rd.rp.cmid)) %>% ggplot(aes(x=report_time, y=c, fill=factor(rd.rp.stid))) + geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + facet_wrap(~ city, nrow=2) + scale_fill_discrete(labels=stid_reports, name="SS")
```

```{r}
data %>% filter(rd.rp.stid < 6 & !is.na(rd.rp.stid)) %>% separate(report_date, into=c("report_date","report_time"), sep =" ") %>% mutate(report_time=format(round_time(report_time,5), "%H:%M")) %>% group_by(report_time,city) %>% summarize(average_stid= mean(rd.rp.stid)) %>% ggplot(aes(x=report_time, y=average_stid, shape=factor(city), color=factor(city))) + geom_point(size=3) +
  geom_smooth() +theme(axis.text.x = element_blank())
```

### Alex vs Cairo ( in amount of lazeez ) 

### Males stay late at night than females

# TODO

* The gender based comparisons
